...
import StrMtrForm from './components/RoiCalc/StrMtr';
import FnFForm from './components/RoiCalc/Fix-n-Flip';
import BnHForm from './components/RoiCalc/Buy-n-Hold';

function Roi({ data }) {
  const [value, setValue] = useState(0);
  const { frontmatter } = data;
  const { title, heading, wholesaletitle, strmtrtitle, strmtrsubtitle, fnftitle, bnhtitle } = frontmatter;
      ...
      <div className="container">
        ...
        <Grid container justifyContent="center" className='mt-6'>
          <Grid item xs={12} md={9} >
            <Card>
              <Box sx={{ width: '100%', minHeight: '1230px' }} className='bg-theme-light'>
                <Box  >
                ...
                {value === 2 && (
                  ...
                  {markdownify(strmtrsubtitle, "h5", "text-center font-normal")}<br />
                  <div><StrMtrForm/></div>
                  </>
                )}
                {value === 3 && (
